...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [output] = await db
    .select({ id: studioOutputs.id, notebookId: studioOutputs.notebookId, shareToken: studioOutputs.shareToken })
    .from(studioOutputs)
  ...
  if (output.shareToken) return NextResponse.json({ token: output.shareToken })

  const token = crypto.randomUUID()
  await db.update(studioOutputs).set({ shareToken: token }).where(eq(studioOutputs.id, id))
