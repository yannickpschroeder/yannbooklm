...
import { db } from "@/db"
import { notebooks, sources, childChunks, parentChunks } from "@/db/schema"
...
export async function POST(req: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const [notebook] = await db
    .select({ id: notebooks.id, shareToken: notebooks.shareToken })
    .from(notebooks)
  ...
  const { messages: uiMessages } = body as { messages: ChatMessage[] }

  if (!Array.isArray(uiMessages)) return new Response("Bad Request", { status: 400 })
